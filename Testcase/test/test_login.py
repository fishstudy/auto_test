...
#参数化运行所有用例
@pytest.mark.parametrize('data', OperationExcel.OperationExcel().getExceldatas("Data", "login.xls", 0))#装饰器进行封装用例
...
def test_login_api(data):
    ...
    logger.debug("请求结果："+str(response.json()))

    #响应断言
    code =str(response.json()['code'])
    msg = response.json()['msg']
    assert case_code == code  # 状态码
    assert case_msg in msg  # 响应数据
    logger.debug("断言内容：errcode=>>>>>  " + case_code + "==" + code)
    logger.debug("断言内容：message=>>>>>  " + case_msg + "==" + msg)
    logger.debug("---------------------登录请求结束--------------------------------------------------------------")
