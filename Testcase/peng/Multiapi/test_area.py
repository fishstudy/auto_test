...
def test_init():
    ...
    url = "http://huiminschool.dev.com:8088/api/area/areaInit"
...
def getToken():
    ...
    token_params={"username":"teacherapi","password":"123456"}
    token_url="http://huiminschool.dev.com:8088/v0/api/user/login"
    token_method="POST"
...
def getSeq(token):
    logger.debug("获取seq:")
    seq_url="http://huiminschool.dev.com:8088/api/area/getCsrfTokenRedis"
...
def getArea(token,areaid):
    ...
    headers={"Content-Type":"application/json"}
    headers["token"] = token
    url = "http://huiminschool.dev.com:8088/api/area/detail?id="+str(areaid)+"&code="+str(seq)
    method = "GET"
    logger.debug("url=" + url + ",method=" + method)
    result = HttpRequest.ApiRequest().send_requests(method, url=url, headers=headers)
    result = result.json()
